...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if($type==0) {
		...
		$sql1='SELECT COUNT(*) AS cnt FROM   k2g1_buyer WHERE buyer_id = ?' ;
		$row = sql($sql1,false,$_POST['buyer_id']);
		if($row[0]["cnt"] == 0){
		//登録処理する
		...
		$sql2='SELECT COUNT(*) AS cnt FROM   k2g1_seller WHERE seller_id = ?' ;
		$row = sql($sql2,false,$_POST['seller_id']);
		if($row[0]["cnt"] == 0) {
	...
	if(isset($_GET["hash"])){ 
		...
		// パラメータがない時（単にURLを打った時はindex.phpにリダイレクト）
		...
		<main>
			...
			<?php if($pflg == false || $errflg > 0) : /* 初回（GET）又は エラー有り（POST) */?>
				<div id="div1a" class="div1">
					...
					<hr>		
					<label><input type="radio" name="select" value="hoge1" onclick="formChange();" checked="checked" />購入者用アカウント</label>
					<label><input type="radio" name="select" value="hoge2" onclick="formChange();"  />出品者用アカウント</label>
					...
					<form action="" method="POST" id="buyer_form">
						...
						<input type="submit" name="buyer_btn" id="buyer_btn"  value="次へ進む" >
					</form>

					<!-- 売り手form ---------------------------------------------------------------------------------------------------------------------------------- -->
					<form action="" method="POST" id="seller-form">
						<table border="1" width="100%">
							<tr>
								<th>郵便番号 <span>必須</span></th>
								<td>
						...
						<input type="submit" name="seller_btn" id="seller_btn" value="次へ進む" >
					</form>
				</div>
