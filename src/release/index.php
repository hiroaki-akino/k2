	...
	<script>
		...
		const ajax = function(){
			...
			.fail(function(jqXHR, textStatus, errorThrown ) {
				$(".err").text("通信状況が悪いようです。暫く時間をおいて再検索下さい。");
			})
		...
		// おすすめ商品名がクリックされた時の処理（POST送信）
			function to_item2(no){
			const hidden_tag = document.createElement('input');
			hidden_tag.type	 = "hidden";
			hidden_tag.name	 = "btn";
			hidden_tag.value = "商品詳細";
			event.target.appendChild(hidden_tag);
			const form = document.getElementById("item_name_form2_" + no);
			form.submit();
		}

		// おすすめ出品者名がクリックされた時の処理（POST送信）
		function to_seller2(no){
			const hidden_tag = document.createElement('input');
			hidden_tag.type	 = "hidden";
			hidden_tag.name	 = "btn";
			hidden_tag.value = "出品者詳細";
			event.target.appendChild(hidden_tag);
			const form = document.getElementById("seller_name_form2_" + no);
			form.submit();
		}
	</script>
	<style>
